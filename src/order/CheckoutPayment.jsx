...
const CheckoutPayment = () => {
  ...
  const paymentMethods = [
    { id: "credit-card", name: "Credit/Debit Card", image: Credit },
    { id: "esewa", name: "eSewa Mobile Wallet", image: Esewa },
    { id: "khalti", name: "Khalti by IME", image: Khalti },
    { id: "cod", name: "Cash on Delivery", image: Cod },
  ];
  ...
  const handlePayNow = async () => {
    ...
    const userId = "aa62b313-11dc-400d-aad3-a476c328a0d5"; // replace with real user ID
    const totalAmount = order.subtotal + order.shipping - (order.discount || 0);
    ...
    const orderData = {
      ...
      paid_amount: selectedPayment === "cod" ? 0 : totalAmount,
    ...
    navigate("/success", {
      state: { order: newOrder, payment: selectedPayment },
    });
  };
      ...
      <div className="max-w-6xl mx-auto flex flex-col items-center">
        ...
        <div className="flex-1 bg-white p-6 rounded-lg shadow">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {paymentMethods.map((method) => (
              <div
                key={method.id}
                onClick={() => handlePaymentSelect(method.id)}
                className={`border-2 rounded-lg p-4 flex flex-col items-center cursor-pointer ${
              ...
              >
                <img
                  ...
                  className="w-12 h-12 mb-2"
                />
                <p className="text-sm font-bold text-center">{method.name}</p>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={handlePayNow}
            disabled={!selectedPayment}
            className={`py-2 px-6 rounded-lg w-full font-semibold ${
              selectedPayment
