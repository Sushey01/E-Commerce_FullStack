...
const Category = ({ icon, title, sublist, index, isOpen, toggleCategory }) => {
  ...
  // Show submenu on hover (desktop) or click (mobile)
  const shouldShowSubmenu = isOpen || isHovered;

  return (
    <div
      className="relative w-full bg-white shadow-md group"
    ...
    >
      ...
      <button
        ...
        className="relative border-b-2 border-gray-400 w-full justify-between p-2.5 hover:bg-gray-100 flex items-center ml-2 md:ml-0 text-gray-400 font-Kanit-Fallback font-normal hover:text-gray-600 transition-colors duration-200"
      >
      ...
      {shouldShowSubmenu && (
        <div className="absolute top-0 left-full w-2 h-full z-[998]" />
      )}
      ...
      {shouldShowSubmenu && (
        ...
        >
          {sublist.map((item, idx) => (
            <li
              key={idx}
              className="p-2 px-4 cursor-pointer hover:bg-slate-100 text-gray-700 transition-colors duration-150"
            >
